...
class LinearModel:
    ...
    def __init__(self):
        ...
        self.W = tf.Variable([1.5], dtype=tf.float32)  # 초기값 수정 (스칼라)

        self.b = tf.Variable([1.5], dtype=tf.float32)  # 초기값 수정 (스칼라)

    def __call__(self, X):  # Y는 사용되지 않으므로 제거
...
def main():
    ...
    # 데이터 생성
    x_data = np.linspace(0, 10, 50).reshape(-1, 1)  # shape을 (50, 1)로 변경
    ...
    for epoch_count in range(epochs):
        ...
        real_loss = loss(y_data, linear_model(x_data))
